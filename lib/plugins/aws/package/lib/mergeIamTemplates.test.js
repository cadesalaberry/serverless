...
describe('#mergeIamTemplates()', () => {
  ...
  it('should allow customisation of the IAM policy statements name', () => {
    awsPackage.serverless.service.provider.iamRoleStatementsName =
      'thereby-allowing-custom-names-shorter-than-64-characters';
    awsPackage.serverless.service.provider.iamRoleStatements = [
      {
        Effect: 'Allow',
        Action: ['something:SomethingElse'],
    ...
    return awsPackage.mergeIamTemplates().then(() => {
      ...
      ).to.deep.equal(awsPackage.serverless.service.provider.iamRoleStatementsName);
    });
  });
